...
public class AutenticacionServlet extends HttpServlet {
    ...
    private void login(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        String usernameOrEmail = req.getParameter("username"); // Puede ser username o email
        String password = req.getParameter("password");


        User user = users.get(usernameOrEmail); // Primero intenta con username

        // Si no lo encuentra por username, busca por email
        if (user == null) {
        ...
        if (user != null && verifyPassword(password, user.getPassword())) {
            ...
            String token = Jwts.builder()
                    .subject(user.getUsername()) // Usar username como subject
